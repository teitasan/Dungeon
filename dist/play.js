...
async function main() {
    ...
    process.stdin.on('data', (key) => {
        ...
        if (key === '\u001b[A')
            next = { x: current.x, y: current.y - 1 }; // Up
        else if (key === '\u001b[B')
            next = { x: current.x, y: current.y + 1 }; // Down
        else if (key === '\u001b[C')
            next = { x: current.x + 1, y: current.y }; // Right
        else if (key === '\u001b[D')
            next = { x: current.x - 1, y: current.y }; // Left
        else if (key === '.') {
            // wait/skip turn
        }
        else if (key === '>') {
