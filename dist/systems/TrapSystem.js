...
export class TrapSystem {
    ...
    trapConfigs = new Map();
    rng;
    constructor(dungeonManager, statusEffectSystem, movementSystem) {
     ...
     */
    checkTrapActivation(entity, position) {
     ...
     * Activate a trap
     */
    activateTrap(trap, entity, position) {
        const config = this.trapConfigs.get(trap.type);
        if (!config) {
            return {
                success: false,
     ...
     */
    processTrapEffect(effectConfig, entity, position) {
        let success = false;
        let message = '';
     ...
     */
    applyTrapStatusEffect(entity, effectConfig) {
     ...
     */
    applyTrapSummon(position, monsterLevel) {
        ...
        const validPositions = adjacentPositions.filter(pos => this.dungeonManager.isWalkable(pos) &&
     ...
     */
    applyTrapHungerDrain(entity, amount) {
        if ('hunger' in entity) {
            const player = entity;
     ...
     */
    applyTrapStatDrain(entity, amount) {
        const stats = entity.stats;
        if (!stats)
            return false;
     ...
     */
    applyTrapItemDestruction(entity, count) {
    ...
    detectTraps(entity, position, detectionBonus = 0) {
        ...
        const levelBonus = (entity.stats?.level || 1) * 0.02; // 2% per level
     ...
     */
    disarmTrap(entity, position, disarmBonus = 0) {
        const cell = this.dungeonManager.getCellAt(position);
        if (!cell || !cell.trap) {
            return {
                success: false,
        ...
        const levelBonus = (entity.stats?.level || 1) * 0.05; // 5% per level
     ...
     */
    placeTrap(position, trapType, visible = false) {
        const cell = this.dungeonManager.getCellAt(position);
        if (!cell || cell.trap) {
     ...
     */
    generateTrapMessage(config, entity, effects) {
...
//# sourceMappingURL=TrapSystem.js.map