...
export class ItemSystem {
     ...
     */
    processItemEffect(effect, target, user) {
        let success = false;
        let message = '';
            ...
            case 'cure-status':
                if ('statusEffects' in target) {
                    ...
                    if (effect.statusType) {
                        ...
                        const index = statusEffects.findIndex((s) => s.type === effect.statusType);
                        if (index !== -1) {
     ...
     */
    generateUsageMessage(user, item, target, success) {
     ...
     */
    pickupItem(entity, position) {
        const entitiesAtPosition = this.dungeonManager.getEntitiesAt(position);
        const items = entitiesAtPosition.filter(e => e instanceof ItemEntity);
        if (items.length === 0) {
            return {
                success: false,
                item: null,
     ...
     */
    dropItem(entity, itemId, position) {
     ...
     */
    createItem(templateId, position) {
        const template = this.itemTemplates.get(templateId);
        if (!template) {
            return null;
        }
        const item = new ItemEntity(`${templateId}-${Date.now()}`, template.name, template.itemType, position, template.identified, template.cursed);
