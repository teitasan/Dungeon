...
export class MultipleDungeonSystem {
     ...
     */
    getUnlockedDungeons(player) {
        return this.getAllDungeons().filter(dungeon => this.isDungeonUnlocked(dungeon.id, player));
    }
    /**
     * Get dungeon by ID
     */
     ...
     */
    isDungeonUnlocked(dungeonId, player) {
     ...
     */
    selectDungeon(dungeonId, player) {
        const dungeon = this.dungeonDefinitions.get(dungeonId);
        if (!dungeon) {
            return {
                success: false,
     ...
     */
    checkUnlockCondition(condition, player) {
        switch (condition.type) {
            case 'level':
                return player.stats.level >= condition.value;
            case 'clear-dungeon':
                const dungeonProgress = this.getPlayerProgress(condition.value);
    ...
    giveRewards(rewards, player) {
        for (const reward of rewards) {
            switch (reward.type) {
                case 'gold':
                    player.gold = (player.gold || 0) + (reward.amount || 0);
                    break;
                case 'experience':
     ...
     */
    ensureTemplateForDungeon(dungeon) {
...
//# sourceMappingURL=MultipleDungeonSystem.js.map