...
export class SpecialRoomSystem {
    dungeonManager;
    roomConfigs = new Map();
    shopItems = new Map(); // roomId -> items
    rng;
     ...
     */
    generateSpecialRooms(floorLevel) {
        const results = [];
     ...
     */
    generateSpecialRoom(config, floorLevel) {
        const dungeon = this.dungeonManager.getCurrentDungeon();
        if (!dungeon) {
            return {
                success: false,
     ...
     */
    generateMonsterHouse(location, floorLevel) {
        const entities = [];
     ...
     * Generate shop
     */
    generateShop(location, floorLevel) {
        const entities = [];
        const shopItems = [];
     ...
     */
    generateTreasureRoom(location, floorLevel) {
        const entities = [];
     ...
     */
    buyFromShop(player, itemPosition) {
        // Find shop item at position
        let shopItem;
        let roomId;
        for (const [id, items] of this.shopItems.entries()) {
            const item = items.find(i => i.position.x === itemPosition.x && i.position.y === itemPosition.y);
            if (item) {
                shopItem = item;
                roomId = id;
                break;
        ...
        // Process transaction
        player.gold = playerGold - shopItem.price;
     ...
     */
    getShopItemAt(position) {
        for (const items of this.shopItems.values()) {
            const item = items.find(i => i.position.x === position.x && i.position.y === position.y);
            if (item)
                return item;
        }
        return undefined;
     ...
     */
    isShopPosition(position) {
        return this.getShopItemAt(position) !== undefined;
    }
    /**
     * Find suitable room location
     */
    findSuitableRoomLocation(dungeonWidth, dungeonHeight) {
        const attempts = 50;
     ...
     */
    getRandomPositionInRoom(roomPos, roomSize) {
        const attempts = 20;
        for (let i = 0; i < attempts; i++) {
     ...
     */
    createRandomMonster(position, floorLevel) {
        const monsterTypes = ['goblin', 'orc', 'skeleton', 'spider', 'rat'];
        const randomType = monsterTypes[Math.floor(this.rng() * monsterTypes.length)];
        const monster = new MonsterEntity(`monster-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`, randomType.charAt(0).toUpperCase() + randomType.slice(1), 'aggressive-hostile', position);
     ...
     */
    createRandomShopItem(position, floorLevel) {
        const itemTypes = ['potion', 'scroll', 'weapon', 'armor', 'food'];
        const randomType = itemTypes[Math.floor(this.rng() * itemTypes.length)];
        const item = new ItemEntity(`shop-item-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`, `${randomType.charAt(0).toUpperCase() + randomType.slice(1)} Lv${floorLevel}`, randomType, position);
        return item;
    }
    /**
     * Create random treasure item (higher quality)
     */
    createRandomTreasureItem(position, floorLevel) {
        ...
        const item = new ItemEntity(`treasure-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`, `Rare ${randomType.charAt(0).toUpperCase() + randomType.slice(1)} +${floorLevel}`, randomType, position);
        return item;
    }
    /**
     * Calculate item price based on type and floor level
     */
    calculateItemPrice(item, floorLevel) {
        const basePrices = {
            'potion': 50,
            'scroll': 100,
            'weapon': 200,
        ...
        const itemTypeKey = item.itemType || item.type;
...
//# sourceMappingURL=SpecialRoomSystem.js.map